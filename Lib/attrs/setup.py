...
def _write_attrs_compat_package(context) -> None:
    files = {
        "Lib/attrs/__init__.py": (
            "# Compatibility wrapper for attrs releases that only ship the attr package.\n\n"
            "from attr import *\n"
            "from attr import __all__\n"
            "from attr import converters, exceptions, filters, setters, validators\n"
        ),
        "Lib/attrs/converters.py": "from attr.converters import *\n",
        "Lib/attrs/exceptions.py": "from attr.exceptions import *\n",
        "Lib/attrs/filters.py": "from attr.filters import *\n",
        "Lib/attrs/setters.py": "from attr.setters import *\n",
        "Lib/attrs/validators.py": "from attr.validators import *\n",
        "Lib/attrs/py.typed": "",
    }
...
def _prepare_attrs_source(context) -> None:
    ...
    attr_src = _resolve_source_entry(extracted_root, "attr")
    attrs_src = _find_direct_source_entry(extracted_root, "attrs")

    _copy_entry(attr_src, context.source_root / "Lib" / "attr")
    if attrs_src is not None:
        _copy_entry(attrs_src, context.source_root / "Lib" / "attrs")
    else:
        _write_attrs_compat_package(context)
...
LIBRARY_INTEGRATION = LibraryIntegration(
    ...
    prepare_source_hooks=[_prepare_attrs_source],
    pre_patch_hooks=[],
    post_patch_hooks=[],
    pre_build_hooks=[],
)
