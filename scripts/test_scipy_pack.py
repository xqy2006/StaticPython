...
class SciPyPackTests(unittest.TestCase):
    ...
    def test_workflow_requires_no_extraction_and_independent_pe_evidence(self) -> None:
        ...
        self.assertIn("$report.verification_mode -ne 'dependency-closure-set'", workflow)
        self.assertIn("$report.closure_verifications", workflow)
        self.assertIn("closure-{0:D4}", workflow)
        self.assertIn("Get-CombinedEvidenceHash", workflow)
