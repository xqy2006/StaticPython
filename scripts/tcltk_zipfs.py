...
static int
staticpython_tkinter_path_exists(const char *path)
{{
    Tcl_StatBuf stat_buffer;
    Tcl_Obj *path_object = Tcl_NewStringObj(path, TCL_INDEX_NONE);
    int result;
    if (path_object == NULL) {{
        return 0;
    }}
    Tcl_IncrRefCount(path_object);
    result = Tcl_FSStat(path_object, &stat_buffer) == 0;
    Tcl_DecrRefCount(path_object);
    return result;
}}
...
{{
    ...
    if (!staticpython_tkinter_path_exists(staticpython_tcl_library)) {{
