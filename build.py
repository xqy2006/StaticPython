...
def _write_runtime_resource_group_module(
    ...
    *,
    ...
    return len(target_records), len(blob_records), len(shard_module_names)
...
def write_runtime_resource_module(source_root: Path, integrations: list) -> None:
    ...
    group_records: list[tuple[str, str]] = []
    target_count = 0
    blob_count = 0
    shard_count = 0
    for group_prefix, group_resource_files in sorted(grouped_files.items()):
        ...
        targets, blobs, shards = _write_runtime_resource_group_module(
            source_root,
            group_prefix=group_prefix,
            group_module_name=group_module_name,
            resource_files=group_resource_files,
        )
        group_records.append((group_prefix, group_module_name))
    ...
    group_lines = [
        ...
        for prefix, module_name in group_records
    ]
